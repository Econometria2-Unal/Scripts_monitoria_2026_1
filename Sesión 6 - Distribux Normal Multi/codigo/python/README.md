# Simulación de la distribución normal multivariada en Python

## Introducción

Este directorio contiene el material en Python de la sesión dedicada a la **distribución normal multivariada**, con énfasis en la simulación de una normal bivariada correlacionada.
...
|---|---|
| `simulacion_normal_multivariada.py` | Script principal. Es el archivo que deben leer, ejecutar y estudiar con cuidado. Allí se desarrolla la simulación de la normal multivariada y se generan las comparaciones y visualizaciones. |
| `funciones_auxiliares_distribux_normal_multivariada.py` | Script auxiliar. Contiene funciones de apoyo para comparar resultados, construir gráficas y exportar archivos HTML. No es necesario que lo entiendan en detalle ni que lo modifiquen. Solo deben usarlo. |
...
La carpeta `__pycache__`, si aparece, es creada automáticamente por Python. No es parte conceptual del ejercicio y puede ignorarse para la lectura de la sesión.
...
## Objetivo del script `simulacion_normal_multivariada.py`

El objetivo de `simulacion_normal_multivariada.py` es explicar cómo simular una distribución normal bivariada correlacionada usando tres formas distintas de descomponer la matriz de varianzas y covarianzas:
...
7. Se compara la simulación manual con la simulación producida por `numpy.random.Generator.multivariate_normal()`.
...
El script usa cuatro paquetes principales:

| Paquete | Uso en el script |
|---|---|
| `numpy` | Simulación de números aleatorios, álgebra lineal y simulación directa de normales multivariadas. |
| `pandas` | Organización de vectores, matrices y tablas con nombres de filas y columnas. |
| `scipy` | Evaluación de la función de densidad normal multivariada para las gráficas 3D. |
| `plotly` | Construcción y exportación de gráficas interactivas en 2D y 3D. |

Al comienzo, el script revisa si estos paquetes están instalados. Si falta alguno, muestra un mensaje indicando cómo instalarlo con `pip`.
...
En Python, esta matriz se guarda como un `pandas.DataFrame`, lo que permite conservar nombres de filas y columnas:

```python
Sigma = pd.DataFrame(..., index=media.index, columns=media.index)
```
...
En Python, estas normales se generan con:

```python
normales_estandar = generador_simulacion.normal(size=n_observaciones * 2)
```

Luego se organizan en una matriz de dos columnas:
...
En Python, esta descomposición se implementa con:
...
## Simulación manual frente a NumPy

Una de las partes más importantes del script es la comparación entre:

- La simulación manual hecha paso a paso.
- La simulación realizada con `numpy.random.Generator.multivariate_normal()`.

`Generator.multivariate_normal()` es la herramienta de NumPy para simular distribuciones normales multivariadas. El script la usa como referencia computacional.

Para que la comparación sea justa, el código reinicia un generador con la misma semilla usada para construir la matriz \(Z\). Así, la simulación manual y la simulación de NumPy parten de la misma secuencia base de normales estándar.

La idea es verificar que:

$$
U_{\text{manual}} \approx U_{\text{NumPy}}.
$$
...
### 3. Comparación manual vs. NumPy

La tabla de comparación muestra si los resultados manuales y los de NumPy coinciden numéricamente para cada método:
...
En Python, estas gráficas se construyen con `plotly.graph_objects`, especialmente con objetos como `go.Figure`, `go.Scattergl` y `go.Histogram2dContour`.
...
Para construir la superficie 3D, el script evalúa la densidad normal multivariada sobre una grilla usando:
...
Estos archivos pueden abrirse directamente desde un navegador. Esto es útil porque permite explorar las figuras de manera interactiva, incluso después de cerrar Python.
...
El archivo `funciones_auxiliares_distribux_normal_multivariada.py` contiene funciones que ayudan a:

- Comparar la simulación manual con NumPy.
...
- Manejar rutas de exportación de manera ordenada.
...
> No modificar `funciones_auxiliares_distribux_normal_multivariada.py`.

Los estudiantes solo necesitan saber que el script principal lo importa para usar funciones ya preparadas. Esto permite que `simulacion_normal_multivariada.py` sea más legible y se concentre en la explicación estadística.
...
2. Abrir `simulacion_normal_multivariada.py`.
3. Leer primero los comentarios generales y la tabla de contenidos.
4. Ejecutar el script por bloques, especialmente si usan un editor como VS Code, Spyder o Jupyter con celdas `# %%`.
...
9. Observar la tabla manual vs. NumPy.
...
- Cómo usar `numpy`, `pandas`, `scipy` y `plotly` para conectar teoría estadística con implementación computacional.

## Cierre

Este material busca conectar la teoría de la distribución normal multivariada con una implementación computacional concreta en Python.

El script `simulacion_normal_multivariada.py` debe leerse como una guía pedagógica: primero construye el caso más simple, luego introduce la matriz de covarianzas, después aplica distintas descomposiciones matriciales y finalmente verifica y visualiza los resultados.
